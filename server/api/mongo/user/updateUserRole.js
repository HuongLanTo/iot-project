...
const updateUserInfo = async function updateUserInfo(req, res) {
  ...
  if (roleExist == true) {
    ...
    // } catch (err) {
    //   logger.error("Error update user role\n" + stringify(err));
    //   return res.status(500).send({
    //     responseCode: 0,
    ...
    await User.findByIdAndUpdate({_id: req.params.id} , {role: role})
    .then(async (data) => {
      response = {
        responseCode: 1,
        responseMessage: "UPDATE ROLE SUCCEED",
    ...
    .catch(err => {
      response = {
        responseCode: 0,
        responseMessage: "Cập nhật Role User không thành công, Hệ thống đang bận\n" + err.message,
      }
      actionLog.response = stringify(response);
      logger.error("Error update role user\n" + stringify(err))
      return res.status(500).send(response);
    })
    .finally(() => {
